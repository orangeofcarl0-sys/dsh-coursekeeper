...
import { calibratedEscalationThresholds, type CalibrationOptions } from './calibration.js'
...
export function suggestEscalation(
  ...
  if (episode.route.stallActions >= options.noInformationLimit) {
    const to: Route = route === 'direct' ? 'inspect' : route === 'inspect' ? (contract.vector.uncertainty >= contract.vector.coupling ? 'explore' : 'plan') : route === 'plan' ? 'explore' : 'inspect'
    if (to !== route) return { from: route, to, reason: 'budget-exhausted', detail: 'Current route exhausted its no-progress budget.', hard: true }
  }
  return undefined
}
