...
function isExperience(value: unknown): value is RouteExperience {
...
export class ExperienceStore {
  private readonly rows: RouteExperience[] = []
  ...
  values(): readonly RouteExperience[] { return this.rows }
  ...
  async status(): Promise<ExperienceStoreStatus> {
  ...
  private async load(): Promise<void> {
    if (!this.options.enabled) { this.loaded = true; return }
    try {
      let text = ''
      try { text = await readFile(this.options.path, 'utf8') } catch (error: any) { if (error?.code !== 'ENOENT') throw error }
      const loadedRows: RouteExperience[] = []
      for (const line of text.split('\n')) {
        if (!line.trim()) continue
        try {
          const value = JSON.parse(line)
          if (isExperience(value)) loadedRows.push(value)
