...
export const DEFAULT_BENCHMARK_TOOL_NAMES = ['run_benchmark'] as const
export const DEFAULT_VERIFICATION_TOOL_NAMES = ['build_project', 'run_correctness_test'] as const
export const DEFAULT_FINISH_TOOL_NAMES = ['finish'] as const
...
export function sameArtifact(left: string, right: string): boolean {
  const a = normalizePath(left)
  const b = normalizePath(right)
  const strip = (value: string) => value.replace(/\.(?:[cm]?[jt]sx?|py|rs|go|java|kt|rb|php|cs|cpp|c|h)$/i, '')
  return a === b || a.endsWith(`/${b}`) || b.endsWith(`/${a}`) || strip(a) === strip(b) || strip(a).endsWith(`/${strip(b)}`) || strip(b).endsWith(`/${strip(a)}`)
}

function unique<T>(values: Iterable<T>): T[] { return [...new Set(values)] }
...
function lexicalTokens(text: string): Set<string> {
  ...
  for (let i = 0; i + 1 < cjkChars.length; i++) cjkBigrams.push(`${cjkChars[i]}${cjkChars[i + 1]}`)
...
export function extractSourceDependencies(content: string, artifactPath: string): string[] {
  ...
  for (const pattern of patterns) for (const match of content.matchAll(pattern)) if (match[1]) specs.push(match[1])
  const out: string[] = []
...
export function taskRelation(text: string, artifacts: readonly string[], previous?: PreviousEpisodeContext): TaskRelation {
  if (previous === undefined || CHAT_RE.test(previous.objective)) return 'new'
  if (CLARIFICATION_RE.test(text)) return 'clarification'
  if (NEW_OBJECTIVE_RE.test(text) && !EXTENSION_RE.test(text)) return 'new'
  ...
  if (CORRECTION_RE.test(text) && continuity) return 'correction'
  if (EXTENSION_RE.test(text) && (continuity || CONTINUITY_RE.test(text))) return 'extension'
  if (continuity) return 'continuation'
  return 'new'
}

function complexityScore(text: string, artifacts: readonly string[]): number {
  let score = Math.min(0.28, text.length / 1000)
  if (COMPLEX_RE.test(text)) score += 0.30
  if (artifacts.length > 1) score += Math.min(0.20, artifacts.length * 0.05)
  if (/\b(?:and|then|also|across|all)\b|并且|然后|同时|全部|多个|跨/i.test(text)) score += 0.10
  if (HIGH_RISK_RE.test(text)) score += 0.18
  if (RESEARCH_RE.test(text)) score += 0.12
  return clamp01(score)
}

function riskLevel(text: string, complexity: number): RiskLevel {
  if (HIGH_RISK_RE.test(text) || complexity >= 0.82) return 'high'
  if (complexity >= 0.50) return 'medium'
  return 'low'
}

function riskScalar(risk: RiskLevel): number { return risk === 'high' ? 1 : risk === 'medium' ? 0.55 : 0.15 }

export function estimateTaskVector(text: string, kind: TaskKind, relation: TaskRelation, artifacts: readonly string[], complexity: number, risk: RiskLevel): TaskVector {
...
function acceptanceHints(kind: TaskKind): string[] {
  switch (kind) {
...
function evidencePolicy(kind: TaskKind, relation: TaskRelation): string[] {
  const evidence: string[] = []
  if (relation !== 'new') evidence.push('Preserve continuity with the existing objective and inspect current state before destructive replacement.')
  if (kind === 'fix') evidence.push('Establish relevant evidence about the failure before mutation.')
  if (kind === 'analysis' || kind === 'review' || kind === 'research') evidence.push('Use observations, tests, or source artifacts as the source of truth.')
  if (kind === 'build') evidence.push('Create the deliverable, then verify the final artifact and relevant behavior.')
  return evidence
}

export function classifyTaskContract(rawText: string, previous?: PreviousEpisodeContext): TaskContract {
...
export interface RouteScores { direct: number; inspect: number; plan: number; explore: number }

export function routeScores(contract: TaskContract): RouteScores {
...
export function selectRoute(contract: TaskContract): Route {
  if (contract.kind === 'conversation') return 'direct'
  const scores = routeScores(contract)
  const entries = Object.entries(scores) as Array<[Route, number]>
  entries.sort((a, b) => b[1] - a[1])
  return entries[0]?.[0] ?? 'inspect'
}

export function routeContractForRoute(
  contract: TaskContract,
  route: Route,
  source: RouteContract['source'] = 'auto',
  revision = 1,
): RouteContract {
  const budgets: Record<Route, [number, number]> = {
    direct: [0, 1],
    inspect: [1, 3],
    plan: [2, 4],
    explore: [2, 5],
  ...
  let nextEvidence: string | undefined
  if (route === 'inspect') nextEvidence = contract.artifacts[0] ? `inspect ${contract.artifacts[0]}` : 'inspect the current implementation or failure evidence'
  else if (route === 'plan') nextEvidence = 'inspect the highest-coupling interfaces before committing the design'
  else if (route === 'explore') nextEvidence = 'identify one observation that can discriminate the leading hypotheses'
  else nextEvidence = 'produce the requested artifact, then verify it'
  return {
...
export function defaultRouteContract(contract: TaskContract, revision = 1): RouteContract {
  return routeContractForRoute(contract, selectRoute(contract), 'auto', revision)
}

export function routeRequiresExplicitCommit(route: Route): boolean {
  return route === 'plan' || route === 'explore'
}

export function canSwitchRoute(contract: RouteContract, cause: string): { allowed: boolean; reason: string } {
  ...
  if (contract.evidenceActions >= contract.minEvidenceActions) return { allowed: true, reason: 'minimum evidence commitment satisfied' }
...
export function initialPhase(contract: TaskContract, route: Route, planMode: boolean): import('./types.js').AgentPhase {
  if (contract.kind === 'conversation') return 'converge'
  if (planMode) return route === 'direct' ? 'design' : route === 'explore' ? 'inspect' : route === 'plan' ? 'design' : 'inspect'
  if (route === 'direct') return 'construct'
  if (route === 'inspect' || route === 'explore') return 'inspect'
  return 'design'
}

export function createAcceptanceObligations(contract: TaskContract): AcceptanceObligation[] {
  if (contract.kind === 'conversation') return []
  const obligations: AcceptanceObligation[] = []
  if (contract.kind === 'build') {
    if (contract.artifacts.length > 0) {
      for (const artifact of contract.artifacts) obligations.push({
...
export function staticKernel(): string {
  return [
...
export function controlPacket(contract: TaskContract, route: RouteContract, phase: string, openAcceptance: number, openVerification: number, recovery: boolean, maxChars = 640): string {
  ...
  if (route.hypothesis) parts.push(`H=${route.hypothesis}`)
  if (route.falsifier) parts.push(`K=${route.falsifier}`)
  if (route.nextEvidence) parts.push(`N=${route.nextEvidence}`)
  if (routeRequiresExplicitCommit(route.route) && !route.explicit) parts.push('ACTION=commit route contract before broad mutation')
  parts.push('</ck>')
...
function shellVerificationScope(command: string): readonly string[] | 'workspace' {
...
export function classifyTool(name: string, rawArguments: unknown, options: ToolClassificationOptions = {}): ToolSemantics {
  ...
  let effect: ToolEffect = 'unknown'
  let verificationScope: readonly string[] | 'workspace' | undefined
  if (control.has(normalizedName)) effect = 'control'
  else if (benchmark.has(normalizedName)) effect = 'benchmark'
  else if (verify.has(normalizedName)) { effect = 'verify'; verificationScope = artifacts.length > 0 ? artifacts : 'workspace' }
  else if (finish.has(normalizedName)) effect = 'finish'
  else if (OBSERVE_TOOLS.has(normalizedName) || /^(?:read|list|search|fetch|get)_/.test(normalizedName)) effect = 'observe'
  else if (MUTATE_TOOLS.has(normalizedName)) effect = 'mutate'
...
export function relevantObservation(semantics: ToolSemantics, contract: TaskContract, route: RouteContract): boolean {
  if (semantics.effect !== 'observe') return false
  if (semantics.artifacts.length > 0) {
    if (contract.artifacts.some(target => semantics.artifacts.some(actual => sameArtifact(target, actual)))) return true
    const next = route.nextEvidence?.toLowerCase() ?? ''
    if (semantics.artifacts.some(actual => next.includes(normalizePath(actual)))) return true
  }
  if (contract.artifacts.length === 0) return true
...
export function mutationNeedsExplicitCommit(route: RouteContract): boolean {
  return routeRequiresExplicitCommit(route.route) && !route.explicit
}

export function mutationAllowed(route: RouteContract, relevantEvidenceObserved: boolean): { allowed: boolean; reason?: string } {
  if (mutationNeedsExplicitCommit(route)) return { allowed: false, reason: `${route.route.toUpperCase()} route requires coursekeeper_control commit with hypothesis/falsifier/next_evidence before broad mutation.` }
  if (route.route === 'direct') return { allowed: true }
  if (route.route === 'inspect' || route.route === 'plan' || route.route === 'explore') {
    if (relevantEvidenceObserved || route.evidenceActions >= route.minEvidenceActions) return { allowed: true }
...
function normalizedRecall(value: unknown): number | undefined {
...
function recordValue(record: Record<string, unknown>, names: readonly string[]): unknown {
  const normalized = new Set(names.map(name => name.toLowerCase()))
  for (const [key, value] of Object.entries(record)) if (normalized.has(key.toLowerCase())) return value
  return undefined
}

function benchmarkFromRecord(record: Record<string, unknown>, fallbackId: string): BenchmarkResult | undefined {
  ...
  if (totalQueries === undefined || recall === undefined || qps === undefined) return undefined
  ...
  if (concurrencyValue !== undefined && concurrency === undefined) return undefined
  if (warmupValue !== undefined && warmup === undefined) return undefined
  ...
  const result: Omit<BenchmarkResult, 'specKey'> = {
    benchmarkId,
...
function recordsFrom(value: unknown): Record<string, unknown>[] {
  ...
  while (queue.length && out.length < 32) {
    ...
    out.push(record)
    for (const key of ['result', 'metrics', 'benchmark', 'data', 'summary']) if (key in record) queue.push(record[key])
  }
  return out
}
...
function jsonCandidates(text: string): unknown[] {
  ...
  if (trimmed) try { out.push(JSON.parse(trimmed)) } catch {}
  for (const match of text.matchAll(/```(?:json)?\s*([\s\S]*?)```/gi)) try { out.push(JSON.parse(match[1] ?? '')) } catch {}
  for (const line of text.split('\n')) {
    const candidate = line.trim()
    if (!candidate.startsWith('{') || !candidate.endsWith('}')) continue
    try { out.push(JSON.parse(candidate)) } catch {}
  }
  return out
}

export function parseBenchmarkResult(content: string, meta?: unknown, fallbackId = 'benchmark'): BenchmarkResult | undefined {
...
export function benchmarkSpecKey(result: Omit<BenchmarkResult, 'specKey' | 'qps' | 'recall'> & Partial<Pick<BenchmarkResult, 'qps' | 'recall'>>): string {
  return [
...
export function isFullBenchmark(result: BenchmarkResult, minQueries: number, minRecall: number): boolean {
  return result.totalQueries >= minQueries && result.recall >= minRecall
}

export function compareBenchmarkResults(candidate: BenchmarkResult, baseline: BenchmarkResult, tolerancePercent: number): BenchmarkComparisonResult {
  if (candidate.specKey !== baseline.specKey) return { outcome: 'different-spec' }
  if (baseline.qps <= 0) return { outcome: 'improved', deltaPercent: Number.POSITIVE_INFINITY }
  const deltaPercent = ((candidate.qps - baseline.qps) / baseline.qps) * 100
  if (candidate.qps > baseline.qps) return { outcome: 'improved', deltaPercent }
  if (deltaPercent >= -Math.max(0, tolerancePercent)) return { outcome: 'within-tolerance', deltaPercent }
  return { outcome: 'regressed', deltaPercent }
}

export function verificationEvidenceFromTool(semantics: ToolSemantics, sequence: number, workspaceRevision: number, artifactRevisions: Readonly<Record<string, number>>): VerificationEvidence | undefined {
  if (semantics.effect !== 'verify') return undefined
  const op = semantics.operation?.toLowerCase() ?? semantics.name.toLowerCase()
  const kind: VerificationEvidence['kind'] = /test|pytest|jest|vitest|mocha|cargo test|go test|dotnet test/.test(op) ? 'test'
...
export function hasCommandFailure(content: string): boolean {
  const match = /(?:\[\s*)?(?:exit[ _-]?code)\s*:\s*([0-9]+)(?:\s*\])?/i.exec(content)
  return match !== null && Number(match[1]) !== 0
}

export function isTerminalLlmFailure(failure: unknown): boolean {
