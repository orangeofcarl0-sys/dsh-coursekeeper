...
export function hammingSimilarity64(left, right) {
    try {
...
function artifactCountBin(count) {
    if (count <= 0)
        return 0;
    if (count === 1)
        return 1;
...
export function buildTaskSignature(contract, knownArtifacts = []) {
...
export function routeBucket(signature) {
    if (signature.risk === 'high')
        return 'high-risk';
    if (signature.kind === 'research' || (signature.kind === 'analysis' && signature.uncertainty >= 0.68))
        return 'research-uncertain';
    if (signature.kind === 'fix' && signature.observability >= 0.58 && signature.uncertainty < 0.7)
        return 'fix-observable';
    if (signature.kind === 'fix')
        return 'fix-unknown-root';
    if (signature.kind === 'build' && (signature.coupling >= 0.62 || signature.complexity >= 0.68))
        return 'build-complex';
    if (signature.kind === 'build')
        return 'build-simple';
    if (signature.continuity)
        return 'continuation';
    if (signature.kind === 'review' || signature.kind === 'analysis')
        return 'review-analysis';
    if (signature.complexity >= 0.65)
        return 'complex-generic';
    return 'generic';
}
