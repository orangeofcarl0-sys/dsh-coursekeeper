...
export function domainWeight(current, past, options) {
    ...
    if (current.protocolFingerprint !== past.protocolFingerprint)
        weight *= options.crossProtocolWeight ?? 0;
    if (current.rolloutMode !== past.rolloutMode)
        weight *= options.crossRolloutWeight ?? 0.25;
    return weight;
}
