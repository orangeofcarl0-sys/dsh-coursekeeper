...
export function markArtifactRemoved(workspace, path, sequence) {
...
export function waiveVerificationDebt(workspace, debtId, reason) {
...
function syntheticDebt(workspace, debt) {
    if (debt.artifact === '<workspace>' || debt.artifact === '<shell-mutation>')
        return false;
    const artifact = artifactState(workspace, debt.artifact);
    if (artifact.removed === true)
...
export function cleanupVerificationDebts(workspace, filter = {}) {
    let removed = 0;
    for (const [id, debt] of workspace.verificationDebt) {
        const artifact = artifactState(workspace, debt.artifact);
        ...
        if (filter.removed === true && isRemoved) {
            workspace.verificationDebt.delete(id);
            removed++;
            continue;
        }
        if (filter.synthetic === true && isSynthetic) {
            workspace.verificationDebt.delete(id);
            removed++;
            continue;
        }
        if (filter.waived === true && isWaived) {
            workspace.verificationDebt.delete(id);
            removed++;
            continue;
        }
...
export function createDependentVerificationDebts(workspace, mutatedPath, sequence, conservative = false) {
    const created = [];
