...
export function jspaceAssistKernel(mode) {
    if (mode === 'off')
        return '';
    if (mode === 'lite') {
        return [
...
export function routePersona(route) {
    switch (route) {
        case 'direct':
...
function isPersonaSection(name) {
 ...
 */
export function applyRouterAssist(assembled, route, mode, promoted) {
    const originalSections = [...(assembled.sections ?? [])];
    const originalTools = [...(assembled.tools ?? [])];
    if (mode === 'off' || promoted)
        return { sections: originalSections, tools: originalTools, changed: false };
    ...
    if (mode === 'minimal-first') {
        const core = new Set(['str_replace_editor']);
...
export const NATIVE_CANONICAL_TOOL_PREFIX = ['bash', 'read'];
 ...
 */
export function applyNativeCanonicalProfile(assembled, toolPrefix = NATIVE_CANONICAL_TOOL_PREFIX) {
    const originalSections = [...(assembled.sections ?? [])];
    const originalTools = [...(assembled.tools ?? [])];
    ...
    const deviations = [];
    if (!personaExact)
        deviations.push('persona-not-exact');
    if (!personaFirst)
        deviations.push('persona-not-first');
    for (const name of toolPrefix) {
        if (!originalTools.some(tool => String(tool?.name ?? '') === name))
            deviations.push(`missing-tool:${name}`);
    }
    if (!toolPrefixMatch)
        deviations.push(`tool-prefix-mismatch:${actualPrefix.join(',') || '(empty)'}`);
    if (!auxiliaryToolsAtEnd)
        deviations.push('coursekeeper-aux-tools-not-at-end');
...
export function observeRequestProtocol(messages) {
    ...
    for (const message of messages ?? []) {
        ...
        for (const block of message?.content ?? []) {
            ...
            if (block?.type === 'tool-result') {
                toolResultBlocks++;
                if (block?.source?.callId != null || block?.tool_call_id != null || message?.source?.callId != null)
                    linkedToolResults++;
            }
        }
    }
