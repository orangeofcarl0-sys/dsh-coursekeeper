...
import { DEFAULT_BENCHMARK_TOOL_NAMES, DEFAULT_FINISH_TOOL_NAMES, DEFAULT_VERIFICATION_TOOL_NAMES, COURSEKEEPER_CONTROL_TOOL, COURSEKEEPER_STATUS_TOOL, COURSEKEEPER_VERIFY_TOOL, LEGACY_TRAJECTORY_CONTROL_TOOL, LEGACY_TRAJECTORY_VERIFY_TOOL, classifyTaskContract, classifyTool, fingerprint, isTerminalLlmFailure, } from './core.js';
import { acceptHumanTask, applyTrajectoryControl, applyAdaptiveInitialRoute, applyAdaptiveEscalation, blockerReport, completionBlockers, createGovernorState, currentControlPacket, markHintInjected, mutationGate, rebuildStateFromEvents, refreshPolicyHint, registerToolCall, settleToolCall, verificationPrompt, } from './state.js';
...
export function apply(ctx, inputConfig = {}) {
    ...
    const finalizeExperience = (runtime, forceCompletion) => {
        ...
        const episode = state.episode;
        if (!episode || runtime.experienceRecordedEpisode === episode.id || !config.experienceMemory)
            return;
