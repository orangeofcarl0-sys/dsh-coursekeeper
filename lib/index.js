...
export function apply(ctx, inputConfig = {}) {
    ...
    function installGuard(runtime) {
        if (runtime.guardDispose || config.mode !== 'active' || !sessionEnabled(runtime.agent))
            return;
        try {
    ...
    ctx.on('agent/inbox/claimed', ({ agent, message, turn }) => {
        if (config.mode === 'off' || !sessionEnabled(agent) || message?.source?.kind !== 'user')
            return;
        const runtime = stateFor(agent);
        const state = runtime.governor;
    ...
    ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
        ...
        if (!agent || config.mode === 'off' || !sessionEnabled(agent))
            return assembled;
        const runtime = stateFor(agent);
    ...
    ctx.on('agent/pre-step', async ({ agent }, next) => {
        ...
        if (decision?.kind === 'reject' || config.mode !== 'active' || !sessionEnabled(agent))
            return decision;
        const runtime = stateFor(agent);
        const state = runtime.governor;
    ...
    ctx.on('agent/request', async ({ agent, signal }, next) => {
        ...
        if (config.mode !== 'active' || config.adaptiveReasoning === 'off' || !sessionEnabled(agent))
            return proposed;
        const state = runtime.governor;
    ...
    ctx.on('agent/request-error', async ({ agent, turn, step, provider, failure }, next) => {
        if (config.mode === 'off' || !sessionEnabled(agent))
            return next();
        const runtime = stateFor(agent);
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (!runtime || config.mode === 'off' || !sessionEnabled(runtime.agent))
            return;
        const state = runtime.governor;
        switch (event.type) {
            case 'tool/call': {
    ...
    ctx.on('agent/turn-stopping', async ({ agent, turn, signal }) => {
        if (config.mode !== 'active' || !config.autoVerify || signal?.aborted || !sessionEnabled(agent))
            return;
        const runtime = stateFor(agent);
        const state = runtime.governor;
        if (state.planMode)
