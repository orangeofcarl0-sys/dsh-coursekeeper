...
export const CoursekeeperSettingsSchema = z.object({
...
export const Config = z.object({
    mode: z.union(['off', 'shadow', 'active']).default('active'),
    requireUserOptIn: z.boolean().default(false),
    ...
    dependencyScope: z.union(['resolved', 'conservative']).default('resolved'),
...
function resolvedConfig(input) {
    ...
    return {
        ...
        requireUserOptIn: input.requireUserOptIn ?? false,
...
export function apply(ctx, inputConfig = {}) {
    ...
    const sessionEnabled = (agent) => {
        const runtime = runtimeStates.get(agent);
        if (runtime)
            return runtime.userMode !== 'off';
        return !config.requireUserOptIn && config.mode !== 'off';
    };
    const sessionControlled = (agent) => {
        const runtime = runtimeStates.get(agent);
        if (runtime)
            return runtime.userMode === 'active';
        return !config.requireUserOptIn && config.mode === 'active';
    };
    const setSessionMode = (agent, mode) => {
        const runtime = stateFor(agent);
        if (runtime.userMode === mode)
            return runtime;
    ...
    const stateFor = (agent) => {
        ...
        registerSessionHandle(sessionKey, {
            ...
            setMode: (mode) => { setSessionMode(runtime.agent, mode); },
    ...
    const runSemanticVerifier = async (runtime, signal) => {
        ...
        if (semantic.attempts >= config.maxSemanticVerifierCalls)
            return false;
        if (semantic.infraFailures >= config.maxSemanticVerifierInfraFailures)
            return false;
    ...
    function installGuard(runtime) {
        if (runtime.guardDispose || !sessionControlled(runtime.agent))
            return;
        try {
    ...
    ctx.on('agent/inbox/claimed', ({ agent, message, turn }) => {
        if (!sessionEnabled(agent) || message?.source?.kind !== 'user')
            return;
        const runtime = stateFor(agent);
        const state = runtime.governor;
    ...
    ctx.on('agent/pre-step', async ({ agent }, next) => {
        ...
        if (decision?.kind === 'reject' || !sessionControlled(agent))
            return decision;
        const runtime = stateFor(agent);
        const state = runtime.governor;
    ...
    ctx.on('agent/request', async ({ agent, signal }, next) => {
        ...
        if (!sessionControlled(agent) || config.adaptiveReasoning === 'off')
            return proposed;
        const state = runtime.governor;
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (!runtime || !sessionEnabled(runtime.agent))
            return;
        const state = runtime.governor;
        switch (event.type) {
            case 'tool/call': {
    ...
    ctx.on('agent/turn-stopping', async ({ agent, turn, signal }) => {
        if (!sessionControlled(agent) || !config.autoVerify || signal?.aborted)
            return;
        const runtime = stateFor(agent);
        const state = runtime.governor;
        if (state.planMode)
    ...
    if (config.exposeControlTool) {
        ctx.effect(() => ctx.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                refreshRestriction(runtime);
                ledger.record({ event: 'coursekeeper/control', sessionId: agent.id, action: args.action, source: 'model-tool', result, route: runtime.governor.episode?.route.route, hypothesis: runtime.governor.episode?.route.hypothesis ?? null });
    ...
    ctx.effect(() => {
        ctx.commands.register({
            ...
            handler: (invocation) => {
                ...
                if (!agent)
                    return { kind: 'error', text: 'coursekeeper: no owning agent' };
                const raw = String(invocation?.rawInput ?? '').trim().toLowerCase();
                const sub = raw.split(/\s+/)[0] || 'status';
                const audit = (action, detail) => ledger.record({
                    event: 'coursekeeper/user-command',
                    sessionId: agent.id,
                    action,
                    source: 'user-command',
                    ...(detail ? { detail } : {}),
            ...
            ctx.effect(() => {
                ctx.tools.register(defineTool({
                    ...
                    async execute(args, exec) {
                        const agent = exec?.agent;
                        if (!agent)
                            return JSON.stringify({ ok: false, reason: 'no owning agent' });
                        try {
                            const goals = goalsCtx.goals;
                            const current = goals.get(agent);
                            if (!current || current.phase === 'complete')
                                return JSON.stringify({ ok: false, reason: 'no active goal to complete' });
