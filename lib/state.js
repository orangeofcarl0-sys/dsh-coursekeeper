import { COURSEKEEPER_CONTROL_TOOL, LEGACY_TRAJECTORY_CONTROL_TOOL, canSwitchRoute, classifyTaskContract, classifyTool, compareBenchmarkResults, controlPacket, createAcceptanceObligations, defaultRouteContract, routeContractForRoute, fingerprint, hasCommandFailure, initialPhase, isFullBenchmark, mutationAllowed, normalizePath, parseBenchmarkResult, relevantObservation, extractSourceDependencies, routeRequiresExplicitCommit, staticKernel, verificationEvidenceFromTool, } from './core.js';
import { applyReadback, applyVerificationEvidence, artifactRevisionSnapshot, createVerificationDebtForArtifact, createDependentVerificationDebts, setArtifactDependencies, createWorkspaceMutationDebt, markArtifactRemoved, openAcceptance, openVerificationDebts, pruneVerificationDebts, satisfyAcceptance, updateAcceptanceAfterMutation, updateAcceptanceAfterObservation, waiveAcceptance, } from './debt.js';
...
function semanticVerificationState(contract, route, mode = 'off') {
    ...
    return { required, status: required ? 'pending' : 'not-required', attempts: 0, infraFailures: 0, contradictions: [], nextEvidence: [] };
}
function acceptanceMap(contract) {
    return new Map(createAcceptanceObligations(contract).map(obligation => [obligation.id, obligation]));
}
function phaseFor(contract, route, planMode) {
    return initialPhase(contract, route.route, planMode);
}
function appendRecentEpisode(state) {
    const episode = state.episode;
    if (!episode)
        return;
    ...
    while (state.recentEpisodes.length > 8)
        state.recentEpisodes.shift();
}
function mergeAcceptance(existing, next) {
    for (const obligation of next)
        if (!existing.has(obligation.id))
            existing.set(obligation.id, obligation);
}
export function acceptHumanTask(state, rawText, turn, planMode = state.planMode, inputOptions = {}) {
    ...
    if (newEpisode) {
        ...
        if (contract.relation === 'correction') {
            ...
            if (episode.route.route === 'direct' && contract.complexity >= 0.5) {
                ...
                if (next.route !== previousRoute)
                    episode.transitions.push({ from: previousRoute, to: next.route, reason: 'coupling-discovered', sequence: 0, evidenceActions: episode.route.evidenceActions });
                episode.route = next;
            }
...
export function startBranchWave(state, decision, checkpointKind, checkpointRef) {
    const episode = state.episode;
    if (!episode)
        return { ok: false, message: 'no active episode' };
...
export function registerBranchCandidate(state, candidate) {
    const wave = state.episode?.branching.current;
    if (!wave || !['collecting', 'comparing'].includes(wave.status))
        return { ok: false, message: 'no branch wave is collecting candidates' };
    for (const existing of wave.candidates.values()) {
        if (existing.fingerprint === candidate.fingerprint)
            return { ok: false, message: `candidate ${candidate.id} duplicates ${existing.id}`, duplicateOf: existing.id };
    }
    wave.candidates.set(candidate.id, candidate);
    return { ok: true, message: `candidate ${candidate.id} registered` };
}
export function recordBranchSelection(state, candidateId, reason) {
    const wave = state.episode?.branching.current;
    if (!wave)
        return { ok: false, message: 'no active branch wave' };
    if (!wave.candidates.has(candidateId))
        return { ok: false, message: `unknown candidate ${candidateId}` };
...
export function recordNoValidBranchCandidate(state, reason) {
    const episode = state.episode;
...
export function reopenAfterBranchApply(state, candidate, sequence = 0, inputOptions = {}) {
    const config = options(inputOptions);
    const episode = state.episode;
    const wave = episode?.branching.current;
...
export function settleBranchReverification(state, passed) {
    const wave = state.episode?.branching.current;
    if (!wave || wave.status !== 'applied')
        return;
    wave.requiresReverify = !passed;
    if (passed)
        wave.status = 'applied';
}
export function openAcceptanceCount(state) {
    return state.episode ? openAcceptance(state.episode.acceptance.values()).length : 0;
}
export function openVerificationCount(state) {
    return openVerificationDebts(state.workspace).length;
}
export function benchmarkBlocker(state, inputOptions = {}) {
    const config = options(inputOptions);
    const episode = state.episode;
    if (!episode)
        return undefined;
    ...
    if (!active)
        return undefined;
    if (episode.benchmark.currentRevisionPassed === state.workspace.revision)
        return undefined;
...
export function completionBlockers(state, inputOptions = {}) {
    const blockers = [];
    ...
    for (const debt of openVerificationDebts(state.workspace))
        blockers.push(`Verification debt remains for ${debt.artifact}.`);
    ...
    if (semantic?.required) {
        ...
        if (!current || semantic.status !== 'passed') {
            ...
            if (!userAllowed && !failOpen)
                blockers.push(`Independent semantic verification remains: ${semantic.status}.`);
        }
    }
    const branchWave = state.episode?.branching.current;
    if (branchWave?.status === 'collecting' || branchWave?.status === 'comparing')
        blockers.push(`Verified branch wave ${branchWave.id} is still ${branchWave.status}; finish is blocked until it selects, aborts, or fails cleanly.`);
    if (branchWave?.status === 'selected')
        blockers.push(`Branch candidate ${branchWave.selectedCandidateId ?? '(unknown)'} was selected but has not been applied to the main workspace.`);
    if (state.episode?.recoveryBlocker)
        blockers.push(state.episode.recoveryBlocker);
...
export function canFinish(state, inputOptions = {}) {
    return completionBlockers(state, inputOptions).length === 0;
}
export function currentControlPacket(state, inputOptions = {}) {
    const episode = state.episode;
    if (!episode)
        return undefined;
    const config = options(inputOptions);
    return controlPacket(episode.contract, episode.route, episode.phase, openAcceptanceCount(state), openVerificationCount(state), episode.recoveryBlocker !== undefined, config.maxDynamicHintChars);
}
export function refreshPolicyHint(state, inputOptions = {}, includeKernel = false) {
    const episode = state.episode;
    if (!episode)
        return;
    ...
    if (state.pendingHint === hint)
        return;
    state.policyRevision++;
    state.pendingHint = hint;
}
export function markHintInjected(state) {
    if (!state.episode)
        return;
    if (state.pendingHint?.includes('<coursekeeper-kernel'))
        state.episode.kernelInjected = true;
    state.injectedRevision = state.policyRevision;
    state.pendingHint = undefined;
}
function progress(state, event) {
    const episode = state.episode;
    if (!episode)
        return;
    episode.evidence.push(event);
    while (episode.evidence.length > 128)
        episode.evidence.shift();
    if (event.weight > 0) {
...
function mutationIsRelevant(state, semantics) {
    const episode = state.episode;
    if (!episode)
        return false;
    if (semantics.artifacts.length === 0)
        return episode.contract.artifacts.length === 0;
    if (episode.contract.artifacts.length === 0)
        return true;
...
function relevantEvidenceObserved(state) {
    return (state.episode?.route.evidenceActions ?? 0) >= 1;
}
export function mutationGate(state) {
    const route = state.episode?.route;
    if (!route)
        return { allowed: true };
    return mutationAllowed(route, relevantEvidenceObserved(state));
}
export function registerToolCall(state, callId, name, args, sequence, inputOptions = {}) {
    if (!state.turn)
        state.turn = createTurn(0);
    const config = options(inputOptions);
    ...
    const call = { semantics, sequence, args };
    state.turn.calls.set(callId, call);
    if (state.lastCallSignature === semantics.signature)
        state.repeatedCallCount++;
    else {
        state.lastCallSignature = semantics.signature;
        state.repeatedCallCount = 1;
    }
    return semantics;
}
function recordMutation(state, semantics, sequence, conservative = false) {
    const changedAcceptance = [];
    if (semantics.artifacts.length > 0) {
...
function recordBenchmark(state, semantics, output, meta, sequence, inputOptions) {
    const config = options(inputOptions);
    const episode = state.episode;
    ...
    const record = { result, sequence, workspaceRevision: state.workspace.revision };
...
export function settleToolCall(state, callId, result, sequence, inputOptions = {}) {
    const call = state.turn?.calls.get(callId);
    if (!call || !state.episode)
        return undefined;
    ...
    while (state.episode.resultFingerprints.size > 256)
        state.episode.resultFingerprints.delete(state.episode.resultFingerprints.values().next().value);
    let event;
    if (failed) {
        ...
        for (const artifact of semantics.artifacts) {
            ...
            if (deps.length > 0)
                setArtifactDependencies(state.workspace, artifact, deps);
        }
        ...
        if (relevant) {
            ...
            if (state.episode.route.epistemic === 'unsupported')
                state.episode.route.epistemic = 'plausible';
            event = {
            ...
            if (state.episode.route.route === 'inspect' && state.episode.route.evidenceActions >= state.episode.route.minEvidenceActions)
                state.episode.phase = 'construct';
            else if (state.episode.route.route === 'plan' && state.episode.route.evidenceActions >= state.episode.route.minEvidenceActions)
                state.episode.phase = 'design';
            else if (state.episode.route.route === 'explore')
                state.episode.phase = 'inspect';
        }
        else {
        ...
        if (removal && semantics.artifacts.length > 0) {
            for (const artifact of semantics.artifacts)
                markArtifactRemoved(state.workspace, artifact, sequence);
            event = {
        ...
        if (openVerificationDebts(state.workspace).length === 0)
            state.episode.phase = 'converge';
    }
    ...
    if (event.weight > 0 && !failed)
        state.episode.recoveryBlocker = undefined;
    const config = options(inputOptions);
...
export function applyTrajectoryControl(state, args, sequence = 0, inputOptions = {}) {
    const episode = state.episode;
    if (!episode)
        return { ok: false, message: 'no active episode' };
    ...
    if (action === 'commit' || action === 'reroute') {
        ...
        if (!['direct', 'inspect', 'plan', 'explore'].includes(routeRaw))
            return { ok: false, message: `invalid route ${routeRaw}` };
        ...
        if (route !== priorRoute) {
            ...
            const reason = normalizedCause === 'falsified' ? 'falsifier-hit'
        ...
        episode.route = {
            ...
            stallActions: 0,
            epistemic: String(args['epistemic'] ?? 'plausible'),
            explicit: true,
    ...
    if (action === 'accept' || action === 'waive') {
        ...
        if (!id)
            return { ok: false, message: 'obligation_id is required' };
        const obligation = episode.acceptance.get(id);
        if (!obligation)
            return { ok: false, message: `unknown acceptance obligation ${id}` };
        const evidence = stringArg(args, action === 'accept' ? 'evidence' : 'reason');
        if (!evidence)
            return { ok: false, message: `${action === 'accept' ? 'evidence' : 'reason'} is required` };
        if (action === 'accept')
            satisfyAcceptance(obligation, evidence, true);
        else
            waiveAcceptance(obligation, evidence);
...
export function applyCoursekeeperControl(state, args, sequence = 0, inputOptions = {}) {
    return applyTrajectoryControl(state, args, sequence, inputOptions);
}
export function applyAdaptiveInitialRoute(state, decision, inputOptions = {}) {
    const episode = state.episode;
    if (!episode)
        return;
    ...
    if (route !== episode.route.route) {
        ...
        if (episode.humanRound <= 1 && episode.transitions.length === 0)
            episode.initialRoute = route;
        episode.semanticVerification = semanticVerificationState(episode.contract, episode.route, options(inputOptions).semanticVerifierMode);
        episode.phase = phaseFor(episode.contract, episode.route, state.planMode);
    }
    else if (episode.humanRound <= 1 && episode.transitions.length === 0)
        episode.initialRoute = episode.route.route;
    refreshPolicyHint(state, inputOptions, true);
}
export function applyAdaptiveEscalation(state, to, reason, detail, sequence = 0, inputOptions = {}) {
    const episode = state.episode;
    if (!episode)
        return { ok: false, message: 'no active episode' };
    const from = episode.route.route;
    if (from === to)
        return { ok: false, message: 'route unchanged' };
...
export function blockerReport(state, inputOptions = {}) {
...
export function verificationPrompt(state, inputOptions = {}) {
    ...
    if (acceptance.length) {
        ...
        for (const obligation of acceptance)
            lines.push(`- ${obligation.id}: ${obligation.description}`);
    }
    if (debts.length) {
    ...
    if (state.episode?.recoveryBlocker)
        lines.push(`recovery: ${state.episode.recoveryBlocker}`);
...
function replayControlPacket(state, text) {
    const episode = state.episode;
    if (!episode || !text.includes('<ck>'))
        return;
    ...
    episode.phase = header[2];
}
export function rebuildStateFromEvents(events, inputOptions = {}) {
    ...
    for (const event of events) {
        ...
        if (type === 'user/message') {
            ...
            if (source !== 'user') {
                ...
                if (/^(?:trajectory-governor|coursekeeper)\/kernel/.test(plugin))
                    state.episode && (state.episode.kernelInjected = true);
                if (/^coursekeeper\/(?:kernel-policy|policy|verification|blocker-report)/.test(plugin))
                    replayControlPacket(state, contentText(event?.data?.content));
                continue;
            }
        ...
        if (type === 'tool/call') {
            ...
            if (toolName === COURSEKEEPER_CONTROL_TOOL || toolName === LEGACY_TRAJECTORY_CONTROL_TOOL) {
                applyTrajectoryControl(state, (typeof args === 'object' && args !== null ? args : {}), seq, inputOptions);
            }
            else
                registerToolCall(state, String(event?.data?.callId ?? ''), toolName, args, seq, inputOptions);
            continue;
        }
