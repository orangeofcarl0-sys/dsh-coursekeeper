...
function isGovernorMode(value) {
    return value === 'off' || value === 'shadow' || value === 'active';
}
function isModeRow(value) {
...
export class SessionModeStore {
    ...
    set(sessionId, mode) {
        ...
        while (this.modes.size > this.options.maxInMemory) {
            const first = this.modes.keys().next().value;
        ...
        try {
            ...
            for (const line of text.split('\n')) {
                ...
                try {
                    ...
                    if (isModeRow(row))
                        this.modes.set(row.sessionId, { mode: row.mode, at: row.at });
                    else
                        this.invalidLines++;
                }
                catch {
...
//# sourceMappingURL=session-mode-store.js.map